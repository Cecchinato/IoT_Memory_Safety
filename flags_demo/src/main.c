 ...
 */
static void vulnerable_copy(const char *input)
{
    char buf[16];
    strcpy(buf, input);
    printk("If you read this, canary didn't work.\n");
    printk("copiato: %s\n", buf);
}
...
static void demo_canary(void)
{
    ...
    const char *payload = "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA_OVERFLOW";
